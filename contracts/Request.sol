...
contract Request{
	...
	event RequestGoods ( address pseudonym_id, bytes32 transaction_id, bytes32 search_term);
	...
	function set_customer_details( bytes32 t_id , bytes32 l_id , bytes32 search_name){
		...
		RequestGoods(msg.sender, t_id, search_name);

	}
	
	// Returns the linking id for a transacton id from the request database
	function get_linking_id(bytes32 t_id) constant returns (bytes32) {
